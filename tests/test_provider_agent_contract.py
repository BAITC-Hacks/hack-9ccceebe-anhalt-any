...
def test_unmodified_orchestrator_preserves_raw_with_explicit_context(monkeypatch):
    ...
    # Current orchestrator does NOT attach request ID itself: absence must fail until C adds handoff.
    features.attrs.pop("turbine_id")
    with pytest.raises(orchestrator.ForecastError, match="Model load/inference failed"):
        orchestrator.run_forecast("Kelmarsh 1", "2018-01-01", 3, settings=settings, with_agent=False)
