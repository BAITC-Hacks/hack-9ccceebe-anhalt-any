...
**Текущий статус:** работает автономный end-to-end MVP на синтетическом демо.
В исходном репозитории был только README; кода Person A/B и реальных данных пока нет.
Для production подготовлены контракты, подключаемые модули и кэш. Подключение реальной
погоды и обученной на измерениях модели остаётся задачей интеграции после их получения.
...
ветер, энергия, анализ, флаги и скачивание JSON. Демо включено по умолчанию в UI;
режим явно отмечен. В CLI/API production выбран по умолчанию, demo включается явно.
...
Тесты проверяют интеграцию, единицы/границы мощности, выравнивание времени, пропуски,
...
- Получить и интегрировать ветки Person A/B, реальный архив прогнозов и модель;
  обученную на фактической генерации модель в этом MVP не заменяет demo.
