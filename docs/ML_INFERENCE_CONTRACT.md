...
The context handoff above is implemented in the integration branch: request turbine ID and
validated wind height are assigned after feature building. The native MODE A path is
`src/agent/observed.py`, using `config/kelmarsh_turbines.json`; it preserves 10-minute cadence,
compares raw predictions to actuals, and is exposed in CLI/API/UI. Goldwind remains rejected.
The original model/metadata/holdout are unchanged. See docs/VALIDATION.md for C's verification.
