...
Состояние на 23 сентября 2026 года: строгий интеграционный сценарий реализован, но **реальный прогноз целевой ВЭС ещё не принят**. Не получены файлы организаторов, подтверждённые временные определения, архивный погодный провайдер и обученная целевая модель. Проверки на фикстурах доказывают поведение кода, а не доступность исторических прогнозов или качество Goldwind.
...
| --- | --- | --- | --- |
| История организаторов: март 2023 — 31 января 2026, T1/T2 | Подготовка данных в `src/ml/goldwind_data.py`, `scripts/prepare_goldwind.py`; явная карта файлов и метаданных | `python -m pytest tests/test_goldwind_data.py -q`; [контракт dataset](GOLDWIND_MODEL_CONTRACT.md) | Код подготовки добавлен из PR #3. Исходные файлы отсутствуют; EDA, реальная подготовка и обучение не подтверждены. |
